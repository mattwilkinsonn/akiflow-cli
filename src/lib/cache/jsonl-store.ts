 ...
 */
export async function appendRecords<T>(filePath: string, records: T[]): Promise<void> {
 ...
 */
export async function rewriteRecords<T>(filePath: string, records: T[]): Promise<void> {
  mkdirSync(dirname(filePath), { recursive: true });
  await writeFile(
    filePath,
