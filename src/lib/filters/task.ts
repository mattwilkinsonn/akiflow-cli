...
function weekBucketIntersects(period: number, fromMs: number, toMs: number): boolean {
...
function monthBucketIntersects(period: number, fromMs: number, toMs: number): boolean {
  const year = Math.floor(period / 100);
...
function isoWeekRange(year: number, week: number): { weekStart: Date; weekEnd: Date } {
