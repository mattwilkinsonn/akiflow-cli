...
export interface CleanedCalEntry {
  ...
  attendees: Array<{ email: string; name: string; response: string | null }> | null;
  recurring: { id: string; rule: string } | null;
...
export function toCleanedTaskView(t: Task, ctx: ResolveContext): CleanedTaskView {
...
function buildPlanBucket(t: Task): { unit: "week" | "month"; period: string } | null {
