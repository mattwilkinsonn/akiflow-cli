...
describe("filterEvents", () => {
  test("excludes declined by default", () => {
    const events = [ev({ id: "a", declined: false }), ev({ id: "b", declined: true })];
  ...
  test("--all-day-only filters to date-only events", () => {
    ...
    expect(filterEvents(events, { allDayOnly: true }).map((e) => e.id)).toEqual(["allday"]);
  });
});
...
describe("mergeTimeline", () => {
  test("dedup: event with task_id skips matching task", () => {
    ...
    const tasks = [
      { id: "task1", datetime: "2026-05-21T10:00:00Z", duration: null } as unknown as Task,
    ];
  ...
  test("sorted by start time", () => {
    ...
    const merged = mergeTimeline(events, slots, tasks);
    expect(merged.map((m) => (m.record as { id: string }).id)).toEqual(["t-early", "t-late"]);
  });
});
