...
import { filterTasks, type TaskFilter } from "../../../lib/filters/task";
...
describe("filterTasks — status", () => {
  ...
  test("--status done", () => {
    const tasks = [task({ id: "a", done: false }), task({ id: "b", done: true })];
...
describe("filterTasks — connector / recurring / unplanned", () => {
  test("connector filter", () => {
    ...
    expect(filterTasks(tasks, { connector: "gmail" }).map((t) => t.id)).toEqual(["gmail"]);
    expect(filterTasks(tasks, { connector: "none" }).map((t) => t.id)).toEqual(["native"]);
  });
  ...
  test("recurring filter", () => {
    ...
    expect(filterTasks(tasks, { recurring: true }).map((t) => t.id)).toEqual(["rec"]);
  });
  ...
  test("unplanned (inbox state)", () => {
    ...
    expect(filterTasks(tasks, { unplanned: true }).map((t) => t.id)).toEqual(["inbox"]);
  });
});
