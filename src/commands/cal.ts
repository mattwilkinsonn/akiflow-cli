...
function hasExtendedCalFlags(args: Record<string, unknown>): boolean {
  ...
  if (args.events === false || args.tasks === false || args.slots === false) return true;
  if (args.declined || args["all-day-only"] || args["all-day"] === false) return true;
  if (args.json || args.raw) return true;
  return false;
}

function buildEventFilter(args: Record<string, unknown>): EventFilter & { range: { from: Date; to: Date } } {
  const f: EventFilter = {};
    ...
    : args.date
      ...
      : args.from || args.to
        ? {
            ...
            to: args.to ? new Date(args.to as string) : new Date(8640000000000000),
          }
        : resolveRange("today");
...
async function runMergedCalendar(args: Record<string, unknown>): Promise<void> {
  ...
  if (args.raw) {
    console.log(
      JSON.stringify({ result: merged, next_cursor: null, errors: [] }, null, 2),
    );
    return;
  }
...
function formatMergedTimeline(entries: TimelineEntry[]): string {
  if (entries.length === 0) return "(no events, slots, or scheduled tasks in range)";
  ...
  for (const [day, dayEntries] of byDay) {
    ...
    for (const e of dayEntries) {
      ...
      const end = e.end
        ...
        : "    ";
      const glyph = e.type === "event" ? "📅" : e.type === "time_slot" ? "⏰" : "📌";
      const title =
        e.type === "event"
...
export const cal = defineCommand({
  ...
  args: {
    ...
    events: { type: "boolean", description: "Include events (use --no-events to exclude)" },
    tasks: { type: "boolean", description: "Include scheduled tasks (use --no-tasks to exclude)" },
    slots: { type: "boolean", description: "Include time slots (use --no-slots to exclude)" },
    declined: { type: "boolean", description: "Include declined events" },
    "all-day-only": { type: "boolean" },
    "all-day": { type: "boolean", description: "Include all-day events (use --no-all-day to exclude)" },
    // Output
