...
export const refreshCommand = defineCommand({
  meta: {
    name: "refresh",
    description: "Sync local cache from Akiflow API (delta by default; --rebuild for full)",
  },
  args: {
    rebuild: {
      type: "boolean",
  ...
  run: async ({ args }) => {
    ...
    const summary = args.rebuild ? await rebuild(client) : await refresh(client);
    ...
    console.log(`Cache ${args.rebuild ? "rebuilt" : "refreshed"} in ${elapsed}ms`);
